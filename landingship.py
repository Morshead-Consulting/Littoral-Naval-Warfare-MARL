...
class LandingShip:
    def __init__(self, side, ship_type, position, landing_zone, env):
        ...
        self.landing_zone = landing_zone  # List of replenishment points for the ship
        self.target_list = []
        ...
        self.rcs = 0.9 if ship_type != "small" else 0.7 # radar cross section indicator
        self.successful_engagements = 0
        self.movement_grid = 9

        self.distance_to_landing_zone = math.sqrt((self.position[0] - self.landing_zone[0]) ** 2 + (self.position[1] - self.landing_zone[1]) ** 2)
    ...
    def get_obs(self):
        ...
        # The speed is matched to the other combatants to ensure that state space matches
        observations = np.zeros(len(own_units)*4 + ((self.speed+1)*2 + 1)**2 + 3) # 4 for each own unit (position x and y, radar and missiles), ~7x7 flattened movement grid and one for number of detected targets and one for RAS distance
        ...
        for x in range(start_x, start_x + (self.speed+1) * 2 + 1):
            for y in range(start_y, start_y + (self.speed+1) * 2 + 1):
                if x == start_x or y == start_y or x == start_x + (self.speed+1) * 2 or y ==  start_y + (self.speed+1) * 2:
                    continue
                else:
    ...
    def check_path(self, origin, destination):
        ...
        distance = abs(origin[0] - dest_x) + abs(origin[1] - dest_y) + 1
        ...
        if dest_x < 0 or dest_x > 99 or dest_y < 0 or dest_y > 99:
            ...
            if path is None or len(path) > distance:
                return False
                ...
                for tgt in self.target_list:
                    ...
                    if self.side == "blue":
                        if target not in self.environment.neutralized_units["red"]:
                            destroyed = self.engage(tgt, self.side, salvo=engagement)
                            if destroyed:
                                destroyed_targets += 1
                    else:
...
##############################
    ...
    def engage(self, target, side, salvo):
        hit = False
        ...
        if target is not None:                
            if self.ammo != 0:
                if math.sqrt((target.position[0]-self.position[0])**2+(target.position[1]-self.position[1])**2) < 3:
                    # Main gun engagement
                    hit = True
                    main_gun_salvo = self.ammo*salvo
                    self.ammo = max(0, self.ammo-main_gun_salvo)
                    print(f"{self.side} fired {main_gun_salvo} rounds with main gun from {self.position} to {target.position}. Hit: {hit}")
            else:
                print(f'{self.side} {self.type} has no ammo left!')
                        
        if hit:
            self.environment.engagements.append((self.position, target.position, main_gun_salvo))
