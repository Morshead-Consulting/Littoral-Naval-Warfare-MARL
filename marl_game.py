...
from matplotlib.colors import LinearSegmentedColormap # NEW IMPORT for custom colormap
...
class Game:
    def __init__(self):
        ...
        # Store the resized color image for visualization
        self.grid_image_resized = None
        # Store the grayscale array for visualization
        self.grayscale_image_array = None
    ...
    def define_grid_from_image(self, image_path, grid_size):
        ...
        # Store the resized color image for visualization
        self.grid_image_resized = image.resize((grid_size, grid_size), Image.LANCZOS)

        # Convert the image to grayscale for the binary grid logic
        grayscale_image_pil = self.grid_image_resized.convert("L")
        self.grayscale_image_array = np.asarray(grayscale_image_pil) # Store grayscale array for visualization

        # Create the binary grid based on a threshold
        # Assuming pixel_value < 128 means 'sea' (binary 1) and >= 128 means 'land' (binary 0)
        # This creates the binary map (0s and 1s) for game logic
        self.grid = (self.grayscale_image_array < 128).astype(int)
        # print(f"Distribution of self.grid values: {np.unique(self.grid, return_counts=True)}") # For debugging
    ...
    def visualize_grid(self, show=False, path=None, animation=False):
        ...
        # --- START MODIFICATION FOR SEA COLOR ---
        # Get the original resized color image as a NumPy array for land areas
        display_image_array = np.array(self.grid_image_resized).copy()

        # Create a custom blue colormap for the sea
        # We'll map grayscale values (0-255) to shades of blue
        # Define colors from a very dark blue to a lighter blue
        colors = ["#000040", "#000080", "#0000CD", "#0000FF"] # Very Dark Blue to Blue
        custom_blue_cmap = LinearSegmentedColormap.from_list("myblues", colors, N=256)

        # Identify sea and land pixels using the binary grid
        sea_mask = (self.grid == 1)
        land_mask = (self.grid == 0)

        # For sea pixels: apply the custom blue colormap based on their original grayscale value
        # Get the grayscale values for only the sea pixels
        sea_grayscale_values = self.grayscale_image_array[sea_mask]
        
        # Map these grayscale values to RGB using the custom colormap
        # Normalize grayscale values to 0-1 range for colormap, then scale back to 0-255 RGB
        mapped_sea_colors = custom_blue_cmap(sea_grayscale_values / 255.0)[:, :3] * 255
        
        # Apply these mapped colors to the display_image_array
        display_image_array[sea_mask] = mapped_sea_colors.astype(np.uint8)
        ...
        if show:
            plt.show()
        if path is not None:
            # Corrected path saving as discussed previously
            filename = f"imagen{self.imagen}.png"
    ...
    def visualize_heatmap(self, heatmap, coldmap):
        ...
        resized_image = image.resize((100, 100), Image.LANCZOS) # Changed ANTIALIAS to LANCZOS
